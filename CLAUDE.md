...
- **Search relevance matching** (`tokenize`/`scoreRelevance`/`isRelevant` in `routes/shopping.js`, the parallel `queryKeywords`/scoring block in `routes/prices.js` — two independent implementations, keep both in sync when touching this): three layered fixes here, found by testing "Nintendo Switch 2" and getting a $59 Joy-Con accessory instead of the $879 console. (1) Tokens containing a digit always survive the stop-word/short-token filter now, even at length 1 — dropping "2" from "Switch 2" silently degraded the query to "Switch", matching any Switch-related product. (2) `ACCESSORY_WORDS` (funda, pack, volante, control, cargador, juego, accesorio, memoria, playstand, webcam, etc.) — a product mentioning one of these when the query didn't ask for it gets its relevance score penalized, because accessories/games routinely repeat the base product's full name in their title for SEO and would otherwise tie the real product on relevance and win on price. (3) Query tokens **containing a digit** — not just pure digits — are mandatory (not just score contributors) and must match as a whole word (`\bTOKEN\b`, not substring): covers both generation numbers ("2") and alphanumeric model/spec codes ("S24", "i5" vs "i7", "RTX4060"), and the whole-word requirement stops a bare "2" from matching inside "32GB" or "2024". Stress-tested with ~40 real queries across every category (see commit history) — "notebook i5"/"notebook i7" now correctly require that exact CPU, "playstation 5"/"ps5" no longer surface a charging dock or Portal remote-play device. If you add a new category where model/generation numbers matter, verify these three still hold rather than assuming token-overlap scoring alone is enough. Remaining zero-result or accessory-only cases (Xbox Series X, Samsung Galaxy S24, iPhone 15 Pro) were individually confirmed against the store's own raw search output to be genuine stock gaps, not matching bugs — don't assume every empty/weak result is this algorithm's fault without checking the raw scrape first.
