...
- **Grocery/retail scraper** (`services/scraper.js`, used by `routes/prices.js` and `routes/shopping.js`): scrapes Uruguayan retailers (Disco/Devoto/Géant via one mechanism, Tienda Inglesa via HTML scraping, El Dorado/Tata via their VTEX/Faststore JSON APIs) for price comparison across categories: `supermercado`, `farmacia`, `belleza`, `ropa`, `hogar` (home/electronics). In-memory cache, 30min TTL. `routes/shopping.js`'s `/compare/start` + `/compare/status/:jobId` is an async job pattern (in-memory `jobs` Map, not persisted — job state is lost on server restart) to avoid blocking on slow scrapes; `/compare` (no `/start`) is the older synchronous version kept for backward compat.
  ...
  - **Currency normalization**: `store.currency = "USD"` on a store entry makes `scrapeStore()` convert `price` to its UYU equivalent via `services/exchangeRate.js` (official BCU rate, SOAP web service — no REST/JSON API exists) before caching/returning, moving the original into `originalPrice` + `currency: "USD"`. This means `price` is **always UYU-comparable** everywhere downstream (routes' sorting/summing/min-max-avg logic needed zero changes) — never re-introduce a raw non-UYU `price` for a new store without going through this conversion. `exchangeRate.js` caches the BCU rate for 4h and falls back to the last known-good rate if the BCU call fails, so a BCU outage doesn't take down these 5 stores' results. `routes/shopping.js`'s per-store item breakdown (`storeMap[storeId].items`, distinct from the top-level `cheapest`/`options` fields) reconstructs a narrower object and currently drops `currency`/`originalPrice` — the totals stay correct (already in UYU) but that specific view can't show "originally US$ X" the way the main result cards do.
